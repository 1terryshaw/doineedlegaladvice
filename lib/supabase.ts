...
export async function getCityPageSlugs(): Promise<
  { province_state: string; region_slug: string; lastmod: string }[]
> {
  // Carries a per-city lastmod = MAX(updated_at) over the city's listings, so a
  // facet/enrichment change (which bumps updated_at, backfilled to the true
  // enrichment timestamp) moves the city URL's sitemap lastmod. The old code
  // emitted now() for every city — an always-fresh signal Google learns to ignore.
  ...
  for (const r of rows) {
    ...
    if (!maxTs.has(key)) { maxTs.set(key, ts); order.push({ province_state: r.province_state, region_slug: r.region_slug }); }
    else if (ts > (maxTs.get(key) as number)) maxTs.set(key, ts);
  }
  return order.map((o) => {
    const t = maxTs.get(`${o.province_state}/${o.region_slug}`) || 0;
    return { ...o, lastmod: (t ? new Date(t) : new Date()).toISOString() };
  });
}
